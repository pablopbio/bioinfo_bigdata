...
#Campos a extraer:
#1.- Identificador principal: VERSION XP_.
...
#Iteramos en cada uno de los ficheros buscando las ER definidas.	
	for line in fhandle.readlines():
		...
		if seq.match(linestriped):
			sequence += seq.match(linestriped).group(1).replace(' ', '').upper()+'\n'
...
#Generamos una lista de lineas de secuencia (con.splitlines) y metemos linea a linea en el archivo.
#Lo hacemos para cumplir con los requesitos FASTA: No lineas en blanco y no mas de 80 caracteres.			
			for m in sequence.splitlines():
